...
export default class TemplateController extends Controller {
  async list(ctx) {
    ...
    const data = await this.service.admin.ai.template.list(query);
    ctx.respond(data);
  }
  ...
  async findOne(ctx) {
    ...
    const data = await this.service.admin.ai.template.findOne(id);
    ctx.respond(data);
  }
  ...
  async create(ctx) {
    ...
    const data = await this.service.admin.ai.template.create(body);
  ...
  async update(ctx) {
    ...
    const data = await this.service.admin.ai.template.update(body);
  ...
  async delete(ctx) {
    ...
    const data = await this.service.admin.ai.template.delete(id);
    ctx.respond(data, data ? 0 : 1, data ? "已下架" : "操作失败");
  }

  async categories(ctx) {
    const data = await this.service.admin.ai.template.categoryTree();
    ctx.respond(data);
  }

  async classifyOptions(ctx) {
    const data = await this.service.admin.ai.template.classifyOptions();
    ctx.respond(data);
  }
}
