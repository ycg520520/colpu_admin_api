...
export default class OrdersController extends Controller {
  async list(ctx) {
    ...
    const data = await this.service.ai.orders.list(query);
    ctx.respond(data);
  }
  ...
  async findOne(ctx) {
    ...
    const data = await this.service.ai.orders.findOne(id);
    ctx.respond(data);
  }
  ...
  async refund(ctx) {
    ...
    const data = await this.service.ai.refund.refundRechargeOrder({
      orderId: id,
      mode: body.mode,
      reason: body.reason,
      operatorUid: uid,
  ...
  async close(ctx) {
    ...
    const data = await this.service.ai.orders.closePending(id);
